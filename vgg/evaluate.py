...
def test_video(feature_extractor, video_path):
    # Load the mean file to subtract to the images
    d = sio.loadmat(mean_file)
    flow_mean = d['image_mean']
...
def main():
    ...
    if do_training:
        X2 = test_video(model, '../test/adl-40')
        ...
        if compute_metrics:
            x_images = glob.glob('../test/adl-40/flow_x*.jpg')
            x_images.sort()
            y_images = glob.glob('../test/adl-40/flow_y*.jpg')
            y_images.sort()
            z_images = glob.glob('../URFD_images/NotFalls/notfall_adl-40/frame*.jpg')
            z_images.sort()
            ...
            for i in range(320):
                ...
                f_ax1.imshow(mpimg.imread(z_images[i]))
                f_ax1.axis('off')
                f_ax2.imshow(mpimg.imread(x_images[i]))
                f_ax2.axis('off')
                f_ax4.imshow(mpimg.imread(y_images[i]))
                f_ax4.axis('off')
                f_ax3.set_xlim(-5,325)
                f_ax3.set_xticks(range(0, 340, 40))
                f_ax3.set_ylim(-0.1,1.1)
                f_ax3.set_yticks([0,0.5,1])
                f_ax3.plot(1-predicted.reshape(-1)[:i+1])
                f_ax3.set_xlabel('Frame number')
                f_ax3.set_ylabel('Fall prediction')
                plt.savefig('./video/adl-40/frame_'+str(i).zfill(3)+'.jpg', format='jpg')
                plt.close()
